...
### #37 — Guidebooks are a planner feature; topos are data; the Topo Studio ships first (2026-07-17)
**Decision:** Booklets/guides become a **feature of the (future) multi-user planner**, not a standalone publishing business: generated per covered area, free, with the coverage bar set at **"the classics, verified"** (~15–30 publish/human routes with approach + pitch prose) and a **disclaimer + per-route provenance** printed in every artifact. Coverage is honest scarcity: no booklet where the corpus is thin, and crawled UKC/theCrag prose can never appear in one (distribution = publishing; the scraping agreements forbid it). Schema grew two layers: `031_topo_media.sql` (media / topo / topo_line — crag photos with rights fields, route lines as pixel-coordinate DATA in multi-pitch.com's exact topoData shape) and `032_guide.sql` (guide / guide_section / guide_route / guide_edition — a guide is a curated, ordered, freezable selection, editions are frozen jsonb snapshots à la corpus.json). New **Topo Studio** (`db/tools/topo_studio.py` + `topo_ui.html`, port 8891, separate app so it can't collide with in-flight Curation Studio work): upload a photo (credit + license mandatory; non-owned requires a permission note), pick a route, click to draw line/belays/descent, rendered live in multi-pitch's visual language (red `rgba(204,25,29,.95)` line, pink belays, blue dashed descent). `import_mp_topos.py` seeded all **38 of Dan's existing multi-pitch topos** (photos → `db/uploads/topos/mp/`, gitignored — 92 MB of reproducible copies; geometry → topo_line, matched via `external_ref` source `multipitch`, 38/38 matched).
...
### #39 — JSON is the database: the record replaces Postgres everywhere (2026-07-18)
**Decision:** Michel, after a researched report on file-based integrity: "as soon as we have all the json I do not want to have the db, not even locally with the docker — we keep only json." Executed in full: the corpus lives as **db/record/** (one self-contained JSON document per route + taxonomies/grades/areas/topos), with git as readable history and the versioned S3 bucket as cloud copy + photo store (`db/sync.sh push|pull`). The Curation Studio was re-plumbed onto `db/tools/store.py` — an in-memory JSON store whose write path validates against **JSON Schemas generated from the taxonomy files** (off-vocabulary tag = the old FK violation), keeps the publish⇒human CHECK as an if/then conditional, requires evidence on safety-critical hazards, and lints referential integrity (`lint_record.py`, enforced by `validate-record.yml` CI). API contract preserved exactly — the UI is untouched and the full E2E suite is green; violations that were raw 500s are now readable 422s. **Everything database-shaped was destroyed**: the Aurora cluster + snapshots + CDK bootstrap (final dump taken first — the down.sh backup-refusal rail actually fired during the race and was honored), the nightly dump workflow + gh-db-backup IAM user, and locally the containers, volume, compose file and studio Dockerfile. Docker is no longer needed to run anything. AWS now holds only: the S3 record/media/dumps bucket, the $5 budget alarm, and IAM users asyncloop (+ the retired policy). Kept dormant on purpose: `db/sql/` + `ingest_corpus.py` as the documented escape hatch back to Postgres (breaking points: a second concurrent curator, ~100k routes, transactional bulk rewrites), and `db/backups/*.sql.gz` as the retirement dumps. The crawl frontier was archived to the record as explicitly non-authoritative; the crawler's future queue is a disposable scratch (SQLite/SQS), never a source of truth.
**Why:** a source of truth should not require a server, a password, a support matrix, or £3.70/month of public IPv4. At 221→~30k routes with one curator, files+schema-validation+CI is the stronger governance (research: OpenBeta ships its corpus as JSONL-in-git; Astro content collections validate at build; S3 got conditional writes in 2024; DuckDB queries the files directly). Supersedes #38's cluster-as-record (same principle — cloud is the truth — cheaper body).
**Status:** ✅ Live.

### #40 — Hierarchical record keys + the browser Studio direction (2026-07-18, grilled Q&A)
**Decision (six answers, one per question):** (1) **Path + inner id**: route files live at human-readable 4-level keys — `country/area/crag/route.json` (e.g. `spain/costa-blanca/puig-campana/la-flecha.json`) — with the immutable `id` INSIDE the document; the path is a mutable alias policed by lint, renamed only via a `move` tool that relocates file+photos+references atomically (research: UKC/MP/OpenBeta/theCrag all key by stable id, slug as decoration). (2) **4 levels; sector is an attribute** on the route, not a path segment (UKC divider model — sector names churn most; missing region ⇒ `unsorted` + lint warning). (3) **Lifecycle is a property** (`status`), never a key segment — "publish" compiles the artifact from published routes only; files never move on status flips (unanimous CMS/Jekyll practice; preserves S3 version chains). (4) **The compiled artifact (corpus.json + manifest.json) goes to BOTH git and S3**; the working record leaves git entirely — **S3 versioning is its history** (Michel: "no need [for git], in s3 we can have the versioning"). (5) **Full browser Studio now**: static UI on S3/CloudFront + Cognito + Lambda(Mangum) API over the record bucket — the multi-user editor pulled forward. (6) **Users: Michel + Dan, email+password Cognito** (Google IdP later, needs Michel's console); Studio private, public sees only the published artifact. Photos co-locate under the crag prefix (`…/crag/media/`).
**Why:** locality ("everything is pretty local"), prefix = future booklet contents, S3-native history, and the editor reachable from any device for two curators. Costs stay ~pennies (S3 + CloudFront + Cognito free tier + Lambda $0 — see the 17 Jul API research).
**Status:** ✅ Both phases shipped. Phase A (record restructure to `country/area/crag/route.json`, media co-location, publish pipeline, de-git the record onto S3 versioning) done; Phase B (Cognito/CloudFront/Lambda Studio) **deployed and live** (commit `fe45d23`, 19 Jul): UI `https://driuinui6do86.cloudfront.net`, Cognito pool `eu-west-2_tc4axWcjI` (Michel + Dan email/password), HTTP API with a Cognito JWT authorizer over the S3 record (outputs in `infra/studio-outputs.json`). Same `corpus/tools/curate.py` serves localhost:8890 and Lambda. Follow-on studio work landed on top: security-review fixes (XSS sanitizer, key validation, least privilege), mobile layout, hand-enter/delete climbs, topo S3-media delete, and an API-level E2E suite (auth, tags, status flip, upload→S3, multi-user). Deploys are manual (`infra/build_studio_assets.sh` + `cdk deploy ClimbingAgentStudio`); no CI deploy workflow yet. ⚠ Still verify the Phase-B prereq closed: rotate the asyncloop key + MFA (sec review P0) now that the authenticated endpoint is public.

### #41 — Postgres removed entirely; ingestion consolidated into `ingest/` (2026-07-29)
**Decision:** Michel: "remove postgres as it is confusing to keep it and let's consolidate one way." This **supersedes #39's kept-on-purpose escape hatch** — no more two-ways. Deleted the whole Postgres surface: the dormant escape hatch (`corpus/sql/` 17 files + `corpus/backups/*.sql.gz` retirement dumps), every PG-coupled tool (`ingest_corpus`, `export_record`, `export_topos`, `import_mp_topos`, `ingest_mp`, `migrate_topo_coords`, `export_taxonomy`, `ai_tag` — the last two read PG via `psql`, not psycopg), the superseded PG crawler (`crawl_worker`, `route_mapping`, `seed_openbeta_frontier`, `llm_tag`), and the cloud re-deploy tooling (`infra/stacks/corpus_db_stack.py` + `up.sh`/`down.sh` + `corpus/cloud-apply.sh`/`backup.sh`, and the `CorpusDbStack` line in `infra/app.py`). Dropped `psycopg` from requirements. **The retrieval agent (`agent/search.py`) was rewritten from one parameterized SQL query into in-memory filtering over the JSON store** (`corpus/tools/store.py`) — same public surface (`connect()` now returns the `Store`), same enum-validated tool schema, verified (results/geo/sort/enum-rejection green). Net: **zero Postgres access anywhere in the code.** The Aurora cluster itself was already destroyed under #39 (2026-07-18); this removes the leftover means to bring it back. Kept: `corpus/publish.sh` (record→corpus.json via `build_corpus.py`, no PG) and the non-PG CDK stacks (Auth/Data/Api/Studio/Test — the S3/Lambda cloud Studio of #40).
**Alongside:** the crawler's future queue landed as the JSON-native **crawl frontier** (`ingest/frontier.py`) — the resumable area→crag→route work-ledger #39 anticipated — inside a new **`ingest/`** module (frontier + pluggable sources + LLM-tag → `status: draft`). Scraped drafts land in an S3-keyed holding pen (`record/_ingest/<country>/<region>/<crag>/…`, store loader skips `_`-prefixed dirs), keyed exactly like the curated record but isolated until a human promotes them.
**Why:** one source of truth, one way to read it. The escape hatch's breaking points (a 2nd concurrent curator, ~100k routes) haven't arrived and DuckDB reads the record directly if raw SQL is ever wanted; carrying a dormant Postgres schema + backups + re-deploy scripts was pure cognitive tax. Supersedes the escape-hatch clause of #39; the rest of #39 (JSON is the database) stands.
**Status:** ✅ Live (code). ⚠️ Docs (`corpus/README.md`) rewrite + a confirm that AWS holds no `ClimbingAgentCorpusDb` stack (`aws cloudformation describe-stacks --stack-name ClimbingAgentCorpusDb --region eu-west-2` → "does not exist") the only follow-ups.
