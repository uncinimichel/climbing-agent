...
**Status:** ⚠️ Phase A (record restructure, media co-location, publish pipeline, de-git the record) in progress; Phase B (Cognito/CloudFront/Lambda Studio) designed, not started. ⚠ Prereq for B: rotate the asyncloop key + MFA (sec review P0) before exposing an authenticated public endpoint.
