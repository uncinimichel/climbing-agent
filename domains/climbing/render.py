...
def _chatter_for(name: str):
    ...
    if _CHATTER is None:
        try:
            _CHATTER = json.loads(cache_file("crag-chatter.json").read_text())
        except Exception:
            _CHATTER = {}
...
# Sky vocabulary + icon URLs: one source of truth in core/domain leaf modules,
# so the scorer and this renderer can never label the same code differently.
...
def build_data(ranked, now, banner, ctx, mp_climbs, guidebooks, extra_climbing_data, tag_spec):
    ...
    from . import conditions as _weather
