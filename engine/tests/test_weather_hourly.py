...
Imports the real modules rather than the engine/ compat shims: monkeypatching
a re-exported name would patch the shim's binding, not the global the
function actually calls."""
from core.weather import metrics, providers
from domains.climbing import conditions
...
def test_metrics_split_day_vs_night_before():
    ...
    met = conditions.forecast_metrics(_forecast({
...
def test_night_rain_beats_same_mm_day_rain():
    ...
    s_day = conditions.day_score(61, 4.0, 60, day)
    s_night = conditions.day_score(61, 4.0, 60, night)
...
def test_storm_cap_skipped_when_climbing_hours_dry():
    ...
    assert conditions.day_score(95, 6.0, 70, night) > 15
    wet = {"rain_day": 6.0, "rain_night": 0.0, "wet_hrs_day": 4, "dry_f": 1.0}
    assert conditions.day_score(95, 6.0, 70, wet) <= 15
...
def test_slow_drying_rock_pays_more_for_night_rain():
    ...
    assert conditions.day_score(61, 8.0, 40, fast) > conditions.day_score(61, 8.0, 40, slow)
...
def test_no_split_falls_back_to_daily_totals():
    ...
    assert conditions.day_score(61, 4.0, 60, m) <= 25
...
def test_tide_extremes_keep_venue_local_times(monkeypatch):
    ...
    monkeypatch.setattr(providers, "tides",
                        lambda lat, lon: {"hourly": {"time": times,
                                                     "sea_level_height_msl": levels}})
    out = providers.tide_extremes(0.0, 0.0)
