...
INSERT INTO area (name, kind, grade_context, timezone)
    VALUES ('Scotland', 'country', 'GB', 'Europe/London');
INSERT INTO area (parent_id, name, kind)
    SELECT id, 'Assynt', 'region' FROM area WHERE name = 'Scotland';
INSERT INTO area (parent_id, name, kind, rock_code, geom)
    SELECT id, 'Old Man of Stoer', 'sector', 'sandstone',
           ST_SetSRID(ST_MakePoint(-5.361, 58.258), 4326)::geography
    FROM area WHERE name = 'Assynt';

-- 2. The example route from taxonomy.md § Record shape.
INSERT INTO route (area_id, name, status, geom,
...
INSERT INTO route_hazard (route_id, hazard_code, evidence_span, source_url)
    ...
    FROM route WHERE name = 'Original Route';
...
BEGIN
    BEGIN
        INSERT INTO route (area_id, name, rock_code)
            SELECT area_id, 'Bogus Route', 'kryptonite' FROM route WHERE name = 'Original Route';
...
BEGIN
    BEGIN
        INSERT INTO route_hazard (route_id, hazard_code)
            SELECT id, 'loose' FROM route WHERE name = 'Original Route';
...
DECLARE ctx text;
BEGIN
    SELECT eff_grade_context INTO ctx FROM route_resolved WHERE name = 'Original Route';
...
FROM route_resolved WHERE name = 'Original Route';
